...
namespace System.DJ.MicroService.NetCore
{
    public class MicroServiceMethodImpl : IMicroServiceMethod
    {
        ...
        {            
            ...
            string classPath = "System.DJ.MicroService.TempImpl";
            ...
            string clssName = Guid.NewGuid().ToString().Replace("-", "_");
            MethodInformation mi = new MethodInformation();
            mi.append(ref code, LeftSpaceLevel.one, "namespace {0}", classPath);
            mi.append(ref code, "{");
            ...
            foreach (MethodInfo miItem in ms)
            {
                ...
                if (typeof(void) != miItem.ReturnType)
                {
                    ...
                    {
                        mi.append(ref s, LeftSpaceLevel.four, "return null;");
                    }
                }
            ...
            binDirPath = Path.Combine(DJTools.RootPath, TempImpl.libName);
            binDirPath.InitDirectory();
            codeCompiler.SavePathOfDll = Path.Combine(binDirPath, clssName + ".cs");
